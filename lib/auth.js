...
module.exports = function createAuth(ENV, DIR) {
  ...
  const RESEND_FROM = ENV.RESEND_FROM_EMAIL || 'Momentum <noreply@momentumscanner.com>';
  ...
  // ─── TOKEN STORES (persisted to disk so they survive restarts) ─────────
  const TOKENS_PATH = path.join(DIR, 'data', 'tokens.json');

  function loadTokens() {
    try {
      const raw = JSON.parse(fs.readFileSync(TOKENS_PATH, 'utf8'));
      return { reset: raw.reset || {}, verify: raw.verify || {}, cooldown: raw.cooldown || {} };
    } catch { return { reset: {}, verify: {}, cooldown: {} }; }
  }
  ...
  function saveTokens() {
    try {
      ...
      const payload = {
        reset: Object.fromEntries(_resetTokens),
        verify: Object.fromEntries(_verifyTokens),
        cooldown: Object.fromEntries(_verifyCooldown),
      };
      const tmp = TOKENS_PATH + '.tmp.' + Date.now();
      fs.writeFileSync(tmp, JSON.stringify(payload));
      fs.renameSync(tmp, TOKENS_PATH);
    } catch (err) { console.error('[saveTokens error]', err); }
  }

  const _loaded = loadTokens();
  const now0 = Date.now();
  const _resetTokens   = new Map(Object.entries(_loaded.reset).filter(([, v]) => v.expiresAt > now0));
  const _verifyTokens  = new Map(Object.entries(_loaded.verify).filter(([, v]) => v.expiresAt > now0));
  const _verifyCooldown = new Map(Object.entries(_loaded.cooldown));
